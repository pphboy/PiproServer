...
public class FileServiceImpl extends ServiceImpl<FileMapper, File> implements IFileService {
    ...
    public File getValidFile(Integer fileId) {
        File validFile = getOne(new QueryWrapper<File>().lambda().eq(File::getFileId, fileId).eq(File::getFileStatus, FileStatus.DEFAULT.value));
        // 判断父目录是否是合法的
        if(ObjectUtils.isEmpty(fileDirectoryService.getValidDirectory(validFile.getFileDirectoryId()))) return null;
        return validFile;
    }
